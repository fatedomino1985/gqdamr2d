...
void AmrQGD::initData ()
{
    ...
    [=] AMREX_GPU_DEVICE (int bi, int i, int j, int k) noexcept
    {
		...
		double ux = snew[bi](i,j,k,iux), uy = snew[bi](i,j,k,iuy); 
		snew[bi](i,j,k,iE) = snew[bi](i,j,k,iE_in) * (snew[bi](i,j,k,ira) + snew[bi](i,j,k,irb)) + 0.5 * (snew[bi](i,j,k,ira) + snew[bi](i,j,k,irb)) * (ux * ux + uy * uy);
		// /*
